...
for n in N:
    ...
    init_dict={}
    if model.__class__.__name__=='BPGM':
        init_dict['eta']  =indep_dict['eta'].clone()
        init_dict['Gamma']=unscaled_pop_dict['Gamma'].clone()
    ...
    def train(lambda_reginter,lambda_inter,refit=True,summary_writer_pnopt=None,summary_writer_refit=None):
        ...
        if not pnopt_hat_pattern['vs'].any() and not pnopt_hat_pattern['ci'].any():
            ...
            # swr=torch.utils.tensorboard.SummaryWriter(log_dir=path_n_rep/f'pnopt_pattern_{lambda_reginter:.6f}_{lambda_inter:.6f}')
