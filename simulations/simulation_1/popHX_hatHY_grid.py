...
from newton_indep_model import newton_indep_model
...
for n in N:
    ...
    if tensorboard:
        ...
        sw=None
    indep_dict = {k:v.clone() for k,v in pop_dict.items() if k in ('eta','xi','ThetaDiag')}#model.init_dict(Z,H,Y)
    ...
    # define init
    init_dict={}
    for k in ['eta','xi','ThetaDiag']:
        if k in indep_dict.keys():
            init_dict[k]=indep_dict[k].clone()
    ...
    for i_lambda_reginter, lambda_reginter in enumerate(np.linspace(lambda_reginter_max,0,num=gridsize)):
        for i_lambda_inter, lambda_inter in enumerate(np.linspace(lambda_inter_max,0,num=gridsize)):
            ...
            try:
                ...
                ELL_VAL               = float(model.ell(Zval,Hval,Yval,refitted_hat).cpu().numpy())
                ...
                JR_SA                 = jr_sa(JR_hat)
                JSPN_hat              = torch.cat((refitted_hat['Gamma'],refitted_hat['Psi']),dim = 0)
