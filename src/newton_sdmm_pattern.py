...
Created on Thu Nov 18 19:20:07 2021
	...
	Optimization methods for penalized and reffiting problems for ziPGM family
...
class hessian_fisher(object):
    ...
    def __init__(self,model,param_dict,X,H,Y,damping=1e-6,mineig=1e-30,maxeig=1e30,fisher=False,**kwargs):
        assert X.size(1)==H.size(1)==Y.size(1),'n'
        assert X.size(0)==H.size(0),'p'
    ...
    def process_hessian(self,param_dict):
        ...
        '''
        if self.fisher:
            X,H = self.model.sample(self.Y,param_dict,X0=self.X,H0=self.H,**self.kwargs)
        else:
            X = self.X
            H = self.H
        HH = self.model.cat_hessian(X, H, self.Y, param_dict,symmetric=True)
...
def cat_params(param_dict):
    ...
    cat={}
    if all([k in param_dict.keys() for k in ('Theta','PhiT','Phi','Lambda')]):
        cat['inter']=torch.stack([param_dict[k] for k in ('Theta','PhiT','Phi','Lambda')],-1).unsqueeze(-1)#pxpx4x1
    if all([k in param_dict.keys() for k in ('Gamma','Psi')]):
        cat['reg']=torch.cat([param_dict[k] for k in ('Gamma','Psi')],1).unsqueeze(-1)#px2rx1
    if all([k in param_dict.keys() for k in ('eta','xi')]):
       cat['lin']=torch.cat([param_dict[k] for k in (('eta','xi','ThetaDiag') if 'ThetaDiag' in param_dict.keys() else ('eta','xi'))],1).unsqueeze(-1)#px2x1
    return cat
def uncat_params(cat_param_dict):
    # group according block structure, used to multiply with compute_preconditioning_matrices
    param={}
    if 'inter' in cat_param_dict.keys():
        param['Theta'],param['PhiT'],param['Phi'],param['Lambda']=[v.squeeze(-1) for v in torch.split(cat_param_dict['inter'].squeeze(-1),1,-1)]#pxp,pxp,pxp,pxp
    if 'reg' in cat_param_dict.keys():
        r=cat_param_dict['reg'].size(1)//2
        param['Gamma'],param['Psi']=torch.split(cat_param_dict['reg'].squeeze(-1),r,1)#pxr,pxr
    if 'lin' in cat_param_dict.keys():
        if cat_param_dict['lin'].size(1)==2:
            param['eta'],param['xi']   =torch.split(cat_param_dict['lin'].squeeze(-1),1,1)#px1,px1
        elif cat_param_dict['lin'].size(1)==3:
            param['eta'],param['xi'],param['ThetaDiag']=torch.split(cat_param_dict['lin'].squeeze(-1),1,1)#px1,px1
        else:
...
def prox_reginter(y_reg,y_inter,threshold=0):
    # y_reg   ~ pxrx1
    # y_inter ~ pxpx4x1
    # consider the groups associated to (Gamma_j,Psi_j,(Theta_{jl},PhiT_{jl},Phi_{jl},Lambda_{jl})_{l=1,...,k})
...
def subproblemSDMM( model,x,dx,FisherObj,FisherPF,t,
    ...
    for it in range(max_iter):
        ...
        y_const   = cat_params({**model.project_interacts(**{k:y_const_uncat[k] for k in ['Theta','PhiT','Phi','Lambda','ThetaDiag']  if k in y_const_uncat.keys()}),
                            **model.project_regression(**{k:y_const_uncat[k] for k in ['Gamma','Psi']}),
                            **{k:y_const_uncat[k] for k in ['eta','xi']}
                            })
...
def subproblemSDMM_inter( model,x,dx,FisherObj,FisherPF,t,
    ...
    for it in range(max_iter):
        ...
        pattern['ci']=torch.bitwise_and(torch.outer(pattern['vs'].squeeze(),pattern['vs'].squeeze()),
                                    y_inter['inter'].bool().any(2).squeeze(2).bitwise_and(y_inter['inter'].bool().any(2).squeeze(2).t())) 
    return x,pattern
def pnopt(*,model,# depends on the model, are methods declared in HM.py
         X,H,Y,#data
    ...
    def ell(param_dict):
        return model.ell(X,H,Y,param_dict)
    ...
    pattern_=dict(vs=init_dict['Gamma'].bool().bitwise_or(init_dict['Psi'].bool()).any(1),
                  ci=init_dict['Theta'].bool().bitwise_or(init_dict['PhiT'].bool()).bitwise_or(init_dict['Phi'].bool()).bitwise_or(init_dict['Lambda'].bool()))
    ...
    for it in range(max_iter):
        ...
        dx = model.cat_jacobian(X, H, Y, param_dict)#cat_params(d_ell(model.ell, X, H, Y, param_dict))
...
def pnopt_pattern(*,model,# depends on the model, are methods declared in HM.py
         X,H,Y,#data
         init_dict,#initial value
