...
import numpy as np
import re
regex_outcome     = re.compile('^k100_(binary|continous)')
...
for path_str in paths:
    ...
    data=pd.read_csv(pop_path/'HGP_BMI.csv',header=[0,1],index_col=0)
...
for ig,g in df.groupby([('index',a) for a in['pop', 'hat','fold']]):
    ...
    # g[[('index','i_lambda_reginter'),('index','i_lambda_inter')]].isin(i_to_remove)
...
indx=df_b_inner.set_index(['fold', 'hat','i_lambda_reginter', 'i_lambda_inter']).groupby(['fold', 'hat','i_lambda_reginter', 'i_lambda_inter'])['PREDERR_TEST'].mean().groupby(['fold', 'hat']).idxmin(skipna=True)
df_selected=df_b_outer.set_index(['fold', 'hat','i_lambda_reginter', 'i_lambda_inter']).loc[indx]
df_selected.rename(columns={'PREDERR_TEST':'MSE'},inplace=True)
...
df_vs=df.drop(['MSE'],axis=1).groupby('model').sum(0)
cols=df_vs.columns
...
g.savefig(pop_path/'HGP_BMI_variable_selection_20221025.eps')
# boxplot figure
g1=sns.boxplot(data=df['MSE'].reindex(df.index.get_level_values(0).unique()[g.dendrogram_col.reordered_ind],level=0).reset_index(),x='model',y='MSE',palette=palette,flierprops = dict(markerfacecolor = '0.50', markersize = 1),whis=1.5,showfliers = False)
plt.setp(g1.get_xticklabels(), rotation=30) 
plt.tight_layout()
g1.figure.savefig(pop_path/'HGP_BMI_MSE_20221025.eps')
