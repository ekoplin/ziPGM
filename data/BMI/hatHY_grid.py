...
parser.add_argument('-outcome', choices =['binary','continous','log','continous_log_squared','categorical_5','continous_BMI_AGE','continous_logBMI_AGE'], help ='outcome BMI binary, continous or log(continous)', required=True)# required
...
elif outcome=='categorical_5':
    df[('Outcome','BMIcat')]=pd.Categorical(df.Outcome.BMIcat,ordered=True,categories=['Underweight', 'Normal', 'Overweight', 'Obesity', 'Extreme Obesity'])
    YY_one_hot_encoding=pd.get_dummies(df[('Outcome','BMIcat')]).drop('Normal',axis=1).to_numpy().astype('double')
    YY_one_hot_encoding-=YY_one_hot_encoding.mean(0)
    YY=torch.tensor(YY_one_hot_encoding.T)
    # zz=Zgroup(pd.Categorical(df[('Outcome','BMIcat')]).codes)
    # zz.Fy()
...
y = pd.Categorical(df.Outcome.BMIcat).codes# used in categorical outcome classification
...
y,ytest = y[indx_train],y[indx_test]
...
if ifold==-1:# train indep and compute lambda max
    ...
    if init=='PF':
        ...
        newton_indep_regression_model(model=model,X=Z,H=H,Y=Y,indep_params=indep_with_regression,**{**args_newton,**dict(rtol=args_newton['rtol']/100,damping=args_newton['damping']*10,step=args_newton['step']/10)},summary_writer=sw)
...
for i_lambda_inter, lambda_inter in enumerate(np.linspace(lambda_inter_max,0,num=gridsize)):
    for i_lambda_reginter, lambda_reginter in enumerate(np.linspace(0,lambda_reginter_max,num=gridsize)):
        ...
        try:
            ...
            # AUC in test (joint SDR)
            if refitted_hat['Gamma'].size(1)>1:
                U,S,V=torch.svd(torch.cat([refitted_hat['Gamma'],refitted_hat['Psi']],0))
                U*=S
                Rtrain       = U[:p].t().mm(Z)+U[p:].t().mm(H)
                Rtest        = U[:p].t().mm(Ztest)+U[p:].t().mm(Htest)
            else:
            ...
            elif 'categorical' in outcome:
                clf = LogisticRegression(random_state=0).fit(Rtrain.cpu().numpy().T,y)
                ACCURACY=clf.score(Rtest.cpu().numpy().T,ytest)
                PREDERR_TEST = ACCURACY
            else:
                raise NotImplementedError('unknown outcome')
            # save some results
            if tensorboard:
