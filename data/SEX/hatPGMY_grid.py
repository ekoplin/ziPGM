...
############ transform data
ZZ,indx_k = transform_data(XX, HH, model)

# split data
Y,Ytest = YY[:,indx_train],YY[:,indx_test]
Z,Ztest = ZZ[:,indx_train],ZZ[:,indx_test]
# Ptrain,Pval,Ptest = torch.split(PP,[max(N),Nval,Ntest],dim=1)
...
if ifold==-1:# train indep and compute lambda max
    ...
    if init=='PF':
        ...
        PGM_newton_indep_regression_model(model=model,X=Z,Y=Y,indep_params=indep_with_regression,**{**args_newton,**dict(rtol=args_newton['rtol']/100,damping=args_newton['damping']*10,step=args_newton['step']/10)},summary_writer=sw)
    ...
    if model_name   == 'NPGM':
        lambda_reginter_max*=15
    else:
        lambda_reginter_max*=5
...
for i_lambda_inter, lambda_inter in enumerate(np.geomspace(lambda_inter_max,lambda_inter_max/gridsize,num=gridsize)):
    for i_lambda_reginter, lambda_reginter in enumerate(np.linspace(lambda_reginter_max/gridsize,lambda_reginter_max,num=gridsize)):
        ...
        try:
            ...
            if model_name=='NPGM':# add reference var into the reduction
